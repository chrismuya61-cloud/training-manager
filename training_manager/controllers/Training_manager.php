...
class Training_manager extends AdminController
{
    ...
    public function index() {
        ...
        // Leaderboard Logic (Gamification)
        $all_staff = $this->staff_model->get('', ['active'=>1]); $leaderboard = [];
        foreach($all_staff as $staff) {
            ...
            if(!empty($s_events)){
                ...
                $score = $atts + ($rating*10); // Simple scoring model
                if($score > 20) $leaderboard[] = ['name'=>$staff['firstname'], 'image'=>staff_profile_image($staff['staffid'],['staff-profile-image-small']), 'attendees'=>$atts, 'rating'=>round($rating,1), 'badge'=>($score>100?'gold':($score>50?'silver':'bronze'))];
            }
        }
        usort($leaderboard, function($a,$b){ return $b['attendees'] <=> $a['attendees']; });
        $data['leaderboard'] = $leaderboard;

        // Fetch Events (RBAC implemented)
    ...
    // --- 3. CRUD & Management Actions ---
    public function event($id = '') {
        if ($this->input->post()) {
            if ($id == '') { if(!has_permission('training_manager','','create')) access_denied(); $id = $this->training_model->add($this->input->post()); set_alert('success','Created'); }
            else { if(!has_permission('training_manager','','edit')) access_denied(); $this->training_model->update($this->input->post(), $id); set_alert('success','Updated'); }
    ...
    public function add_walkin() {
        ...
        if($t->price > 0 && !empty($data['email'])) {
            $this->invoices_model->add(['clientid'=>0, 'number'=>get_option('next_invoice_number'), 'date'=>date('Y-m-d'), 'duedate'=>date('Y-m-d'), 'currency'=>$t->currency, 'subtotal'=>$t->price, 'total'=>$t->price, 'billing_street'=>$data['name'], 'newitems'=>[['description'=>'Training Ticket', 'qty'=>1, 'rate'=>$t->price]]]);
        }
    ...
    public function reschedule_attendee($reg_id, $new_id) {
        ...
        // Reset status/quiz/feedback when rescheduling
    ...
    public function sync_to_leads($id) {
        ...
        foreach($attendees as $att){
            if(!$this->db->where('email', $att['email'])->get(db_prefix().'leads')->row()){
                $this->leads_model->add(['name'=>$att['name'], 'email'=>$att['email'], 'phonenumber'=>$att['phonenumber'], 'company'=>$att['company'], 'source'=>$sid, 'status'=>1, 'dateadded'=>date('Y-m-d H:i:s'), 'addedfrom'=>get_staff_user_id()]);
            }
        }
        set_alert('success', 'Synced'); redirect(admin_url('training_manager/event/'.$id));
    }
    ...
    // --- 6. PDF Generation (Private Function) ---
    private function generate_certificate_pdf($att, $training) {
        ...
        // Background and Branding Logic (Safe Mode)
    ...
    public function print_badges($id) { if (!class_exists('TCPDF')) $this->load->library('pdf'); $atts = $this->training_model->get_attendees($id); $pdf = new TCPDF('P', 'mm', 'A4'); $pdf->SetPrintHeader(false); $pdf->SetPrintFooter(false); $pdf->AddPage(); $bg = module_dir_path('training_manager', 'assets/badge_bg.jpg'); $col = 0; $row = 0; foreach($atts as $a){ $x=15+($col*91); $y=15+($row*59); if(file_exists($bg) && filesize($bg)>0 && @getimagesize($bg)) $pdf->Image($bg, $x, $y, 86, 54); else $pdf->Rect($x, $y, 86, 54); $pdf->SetXY($x, $y+20); $pdf->SetFont('helvetica', 'B', 14); $pdf->Cell(86, 0, $a['name'], 0, 1, 'C'); $pdf->write2DBarcode($a['unique_ticket_code'], 'QRCODE,H', $x+65, $y+35, 15, 15); $col++; if($col>1){$col=0; $row++;} if($row>4){$pdf->AddPage(); $col=0; $row=0;} } $pdf->Output('Badges.pdf', 'I'); }
    public function import_attendees($id) { if(isset($_FILES['file_csv'])){ /* ... */ } redirect(admin_url('training_manager/event/'.$id)); }
    public function download_sample_csv() { header('Content-Type: text/csv'); header('Content-Disposition: attachment; filename="sample.csv"'); $f=fopen('php://output','w'); fputcsv($f,['Name','Email','Phone','Company']); fputcsv($f,['John Doe','john@test.com','+123456','Acme']); fclose($f); exit; }
    public function upload_media($id) { if(!empty($_FILES['file']['name'])){ $p=module_dir_path('training_manager','uploads/'.$id.'/'); if(!is_dir($p)) mkdir($p,0755,true); move_uploaded_file($_FILES['file']['tmp_name'], $p.$_FILES['file']['name']); $this->db->insert(db_prefix().'training_media', ['training_id'=>$id, 'file_name'=>$_FILES['file']['name'], 'file_type'=>strpos($_FILES['file']['type'],'image')!==false?'image':'doc']); } }
    public function delete_media($id) { $this->db->where('id',$id)->delete(db_prefix().'training_media'); redirect($_SERVER['HTTP_REFERER']); }
